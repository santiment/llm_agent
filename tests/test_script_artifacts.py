...
never model context. The second half of the same contract: code an agent ran INLINE
through `execute` (a heredoc, or the `python3 -c` a model reaches for anyway) writes no
file, so it needs its own capture or nothing on the event stream says code ran at all.
...
from deep_research_agent.script_artifacts import (MAX_CODE_CHARS, MAX_OUTPUT_CHARS,
                                                  ExecuteArtifactsMiddleware,
                                                  ScriptArtifactsMiddleware, inline_code,
...
# ---- inline code: the `execute` calls no file replay can see -----------------------------
# The gap this pins, seen live: the orchestrator computed a square root with
# `python3 -c "…"`, reported the exact digits — and the user saw NOTHING saying code ran.
# `execute` is a deepagents built-in (never wrapped by events.instrument_tool) and inline
# code writes no file (nothing for replay_scripts to find), so no event existed at all.

_LIVE_COMMAND = (
    'python3 -c "import math; n=9238123123123123123123123; r=math.isqrt(n); '
    "print('exact isqrt:', r)\"")
_LIVE_OUTPUT = "exact isqrt: 3039428091454\n\n[Command succeeded with exit code 0]"


def _exec_req(command: str, tool: str = "execute") -> ToolCallRequest:
    return ToolCallRequest(tool_call={"name": tool, "args": {"command": command}, "id": "c1"},
                           tool=None, state={}, runtime=None)


def _run_exec(mw, request, output: str = "out"):
    async def handler(_request):
        return ToolMessage(output, tool_call_id="c1", name="execute")
    return asyncio.run(mw.awrap_tool_call(request, handler))


def test_inline_code_finds_a_program_and_ignores_everything_else() -> None:
    assert inline_code(_LIVE_COMMAND)[0] == "python"
    assert inline_code("python3 - <<'PY'\nimport json\nprint(1)\nPY") == (
        "python", "import json\nprint(1)")
    assert inline_code("/usr/bin/python3 -u -c 'print(3)'") == ("python", "print(3)")
    # A command that only RUNS a file is the replay's job; a shell utility call is no script.
    assert inline_code("python /workspace/corr.py") is None
    assert inline_code("ls /workspace") is None
    assert inline_code("python3 - <<'PY'\nPY") is None   # empty body
    assert inline_code("") is None


def test_an_inline_calculation_reaches_the_ui_with_its_real_output() -> None:
    mw = ExecuteArtifactsMiddleware("orchestrator")
    with capture_events_cm() as captured:
        result = _run_exec(mw, _exec_req(_LIVE_COMMAND), _LIVE_OUTPUT)
    ev = next(e for e in captured if e["type"] == "script")
    assert ev["agent"] == "orchestrator" and ev["language"] == "python"
    assert ev["name"] == "inline.py" and not ev["truncated"]
    assert ev["code"].startswith("import math") and "math.isqrt" in ev["code"]
    assert "python3 -c" not in ev["code"]              # the shell wrapper is not the program
    assert ev["output"] == _LIVE_OUTPUT                # evidence it RAN, not an estimate
    assert not ev["output_truncated"]
    assert EVENT_SCHEMAS["script"] <= ev.keys()
    assert result.content == _LIVE_OUTPUT              # the model's result is untouched


def test_the_sync_path_emits_the_same_event() -> None:
    mw = ExecuteArtifactsMiddleware("research-subagent")
    with capture_events_cm() as captured:
        mw.wrap_tool_call(_exec_req("python3 -c 'print(1)'"),
                          lambda _r: ToolMessage("1", tool_call_id="c1", name="execute"))
    assert [(e["agent"], e["code"]) for e in captured] == [("research-subagent", "print(1)")]


def test_no_event_for_a_file_run_a_shell_call_or_another_tool() -> None:
    mw = ExecuteArtifactsMiddleware("coding-subagent")
    for command in ("python /workspace/corr.py", "ls /workspace"):
        with capture_events_cm() as captured:
            assert _run_exec(mw, _exec_req(command)).content == "out"
        assert captured == []                          # replay_scripts covers the file case
    with capture_events_cm() as captured:
        assert _run_exec(mw, _exec_req("x", tool="write_file")).content == "out"
    assert captured == []


def test_a_failing_call_still_shows_what_ran() -> None:
    # "It ran and broke" is as informative as a clean run — and the error must propagate.
    ...
    with capture_events_cm() as captured:
        try:
            asyncio.run(mw.awrap_tool_call(_exec_req("python3 -c 'print(1)'"), boom))
            raised = False
        except RuntimeError:
            raised = True
    assert raised
    assert captured[0]["code"] == "print(1)"
    assert captured[0]["output"] == "RuntimeError: sandbox gone"


def test_output_is_capped_to_a_head() -> None:
    # The tab is evidence, not a data channel — the extract worker prints row slices.
    mw = ExecuteArtifactsMiddleware("orchestrator")
    with capture_events_cm() as captured:
        _run_exec(mw, _exec_req("python3 -c 'print(1)'"), "x" * (MAX_OUTPUT_CHARS + 500))
    assert len(captured[0]["output"]) == MAX_OUTPUT_CHARS
    assert captured[0]["output_truncated"]
...
if __name__ == "__main__":
    ...
    test_inline_code_finds_a_program_and_ignores_everything_else()
    test_an_inline_calculation_reaches_the_ui_with_its_real_output()
    test_the_sync_path_emits_the_same_event()
    test_no_event_for_a_file_run_a_shell_call_or_another_tool()
