...
|---|---|---|---|---|---|
| `extra-low` | `deepseek/deepseek-v4-flash` (0.09/0.18) | `deepseek/deepseek-v4-flash-0731` (0.07/0.18) | `deepseek/deepseek-v4-flash-0731` | `deepseek/deepseek-v4-flash-0731` | `z-ai/glm-5.3-flash` (0.07/0.25) |
| `low` | `qwen/qwen3.8-27b` (0.42/2.55) | `deepseek/deepseek-v4-flash` (0.09/0.18) | `deepseek/deepseek-v4-flash-0731` | `openai/gpt-5.6-luna` (0.20/1.20) | `openai/gpt-5.6-luna` |
| `mid` | `google/gemini-3.7-flash` (0.75/3.75) | `deepseek/deepseek-v4-flash` | `google/gemini-3.5-flash-lite` (0.30/2.50) | `openai/gpt-5.6-luna` | `google/gemini-3.6-flash` (0.75/3.75) |
...
`extra-low` is rock bottom: one model family end to end (planner and fleet cost about the same), so delegation pays off only via context isolation. Expect weaker planning and earlier give-ups than higher tiers; the force-completion / findings-gate / budget backstops keep runs honest, not great. For demos, smoke tests, and high-volume low-stakes ticks — not for decisions. Every slot in this tier matches the `deepseek-v4-flash` streaming-denylist substring (the `-0731` build included), so nothing in it streams. `low` buys a real planner (`qwen3.8-27b`, τ² 78.7%, but ~5 min per task median — slow). `mid` puts τ²'s #2 model overall behind the planner at a flash price. `high` swaps `claude-sonnet-5` for `gpt-5.6-sol` (same price and τ² score, +8.7pp GPQA, half the latency) over a fleet that is itself a stronger tool-caller than the planner — intended, since the fleet makes the calls — with the utility slot picked for context length rather than reasoning, since its job is input-heavy map/extract. New models (`gemini-3.8-flash`, released 2026-09-02) were skipped until they appear on the benchmarks.
