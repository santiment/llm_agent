...
| `script` | `id`, `agent`, `name` (basename), `language`, `code`, `truncated` | **Collapsed "view script" tab** holding the code a sub-agent wrote — the ONLY place a script surfaces. Emitted once per script at the sub-agent's handoff, carrying the final version (initial `write_file` plus every later `edit_file` replayed), so the tab shows the code that actually ran |
...
- **Scripts are artifacts, not paths.** No role ever names a script in prose. The code goes to the frontend on its own `script` event (basename, language, final source) for a collapsed "view script" tab, emitted by `script_artifacts.py` from the sub-agent's own `write_file` / `edit_file` calls — app-code channel, never model context. The coder's handoff is then scrubbed deterministically (`SCRIPT:` line, `/workspace/...` paths, bare `name.py` mentions) *before* it becomes the orchestrator's tool result, because a path the planner reads is a path the report repeats — and the reader can neither open the sandbox nor find it after the run ends. One exception survives the scrub: a labeled `RESULT FILE:` line, the output file too large to print that the caller must hand to the extract worker. `report_hygiene.scrub_report` remains the last-mile net on the report itself.
...
src/deep_research_agent/
  ...
  script_artifacts.py scripts -> `script` event (UI tab); scrubs paths from the coder's handoff
