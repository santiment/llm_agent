...
| `DRA_PROMPT_CACHING` | `true` | Inject `cache_control` breakpoints (system prompt + a rolling conversation prefix) so the re-sent prefix is cached — big input-token savings. Applies to the models in `DRA_CACHE_BREAKPOINT_MODELS`; providers that cache automatically need nothing |
