...
| `DRA_MAX_RETRIES` | `3` | SDK-level retries per model call on transient 429/5xx (sub-second, doubling to 8 s — blips); `0` disables |
| `DRA_MODEL_RATE_LIMIT_MAX_WAIT` | `120` | Backoff budget (seconds) per model call for a throttled / erroring provider: the role waits (Retry-After, else capped exponential) and calls again until cumulative waiting would exceed it, then the error stands; `0` = no waiting beyond the SDK's |
...
| `DRA_PROVIDER_MIN_THROUGHPUT` | `50` | OpenRouter routing: deprioritize provider endpoints whose p50 throughput is under this many tokens/s. Soft — the price-weighted balancing continues among the endpoints that qualify, nothing is excluded; `0` disables. See Provider routing below |
| `DRA_PROVIDER_MAX_LATENCY` | `0` | Same, for p50 time-to-first-token in seconds; `0` disables |
| `DRA_PROVIDER_SORT` | — | Hard override: `price` \| `throughput` \| `latency` takes the top endpoint on that axis and switches OpenRouter's load balancing off (price weighting included) |
| `DRA_PROVIDER_MAX_PRICE_FACTOR` | `1.25` | Hard price cap per model: endpoints costing more than this x the cheapest *healthy* endpoint (per axis, $/M) are refused — even when the cheap one is slow. Read from the live feed at graph start; `0` disables |
| `DRA_PROVIDER_MIN_UPTIME` | `97` | Stability floor (% uptime over the last 30 min): endpoints below it, or flagged down, don't anchor the price cap and their provider is skipped via `ignore` when it has no healthy endpoint; `0` disables |
| `DRA_PROVIDER_ROUTING_TTL` | `300` | Seconds the per-model endpoint feed is cached across graph builds; `0` = read every run |
...
| `DRA_STREAMING_DENYLIST` | `deepseek-v4-flash,deepseek-v4.1-flash` | Comma-separated model-name substrings that force `streaming` off |
...
Per-run `configurable` keys mirror these: `model_tier`, `apiKeys.{OPENAI_API_KEY,TAVILY_API_KEY}`, `base_url` (allowlisted only), `temperature`, `max_output_tokens`, `request_timeout`, `max_retries`, `model_rate_limit_max_wait`, `provider_min_throughput`, `provider_max_latency`, `provider_sort`, `provider_max_price_factor`, `provider_min_uptime`, `provider_routing_ttl`, `search_max_results`, `mcp_servers` / `mcp_config`, `mcp_prompt`, `mcp_max_concurrency`, `mcp_rate_limit_max_wait`, `mcp_tools_ttl`, `domain_prompt`, `skills_dir`, `custom_tools_dir`, `streaming`, `streaming_denylist`, `recursion_limit`, `max_tool_calls`, `max_total_tokens`, `max_run_seconds`, `compaction_tokens`, `prompt_caching`, `web_fetch`, `max_result_chars`, `max_result_rows`, `offload_results`, `offload_dir`, `sandbox_url`, `sandbox_token`, `sandbox_network`, `sandbox_session_timeout`.

### Model tiers (price packages)

Models are chosen by NAME only: `DRA_MODEL_TIER=mid` (or per-run `configurable.model_tier`). Which models a name means is decided in code — `MODEL_TIERS` in `config.py`, one reviewed place — and is **not** settable per env var or per run; legacy per-model keys (`research_model`, `final_report_model`, `compression_model`, …) are ignored with a warning. **The default, when nothing is configured, is `extra-low`** — a bare checkout can't silently burn money; opt up explicitly for real work. An unknown tier name warns and falls back to the default. OpenRouter slugs, prices $/M input/output verified live 2026-09-10 (they drift — re-check before relying on them):

| Tier | Research (orchestrator) | Sub-agent | Utility (extract) | Compaction | Coding |
|---|---|---|---|---|---|
| `extra-low` | `deepseek/deepseek-v4-flash-0731` (0.07/0.18) | `deepseek/deepseek-v4-flash-0731` (0.07/0.18) | `deepseek/deepseek-v4-flash-0731` | `deepseek/deepseek-v4-flash-0731` | `deepseek/deepseek-v4.1-flash` (0.30/1.20) |
| `low` | `deepseek/deepseek-v4.1-flash` (0.30/1.20) | `deepseek/deepseek-v4-flash-0731` (0.07/0.18) | `deepseek/deepseek-v4-flash-0731` | `openai/gpt-5.6-luna` (0.20/1.20) | `openai/gpt-5.6-luna` |
| `mid` | `google/gemini-3.8-flash` (0.75/3.75) | `deepseek/deepseek-v4.1-flash` (0.30/1.20) | `deepseek/deepseek-v4-flash-0731` | `openai/gpt-5.6-luna` | `google/gemini-3.8-flash` |
| `high` | `openai/gpt-5.6-sol` (2/10) | `google/gemini-3.8-flash` (0.75/3.75) | `deepseek/deepseek-v4-flash-0731` | `google/gemini-3.8-flash` | `deepseek/deepseek-v4-pro-0813` (1.05/3.15) |

**Four invariants every package keeps, all asserted in `tests/test_model_tiering.py`:** the sub-agent is never pricier than the orchestrator (the fleet makes most of the tool calls, so a fleet at planner prices makes the tier's cost the *fleet's* cost), the utility model is never pricier than the sub-agent (it is the floor: input-heavy map/extract over offloaded files, where context length matters and judgment does not), each tier costs more than the one below it on the research slot, and each tier's sub-agent is priced at or above the planner of the tier below (stepping up a tier upgrades the gathering, not just the plan — in practice the fleet *is* the previous tier's planner, a staircase). The tests parse the `# $in / $out` comments beside each slug in `MODEL_TIERS`, so those comments are load-bearing — change a model without its price and the suite fails. The **Coding** and **Compaction** slots sit outside the fleet-price rule: the coder gets a *small* input (a goal, file paths, code + error) and writes or fixes one script, and the compactor summarizes a transcript a few times per run at most, so their per-token price barely shows — both are picked for quality. The coder is asserted to be a distinct model, never a silent alias of the fleet slot.
...
`extra-low` is rock bottom: one model family end to end (planner and fleet cost about the same), so delegation pays off only via context isolation. Expect weaker planning and earlier give-ups than higher tiers; the force-completion / findings-gate / budget backstops keep runs honest, not great. For demos, smoke tests, and high-volume low-stakes ticks — not for decisions. Every slot in this tier is on the streaming denylist (`deepseek-v4-flash` covers the `-0731` build, `deepseek-v4.1-flash` the coder), so nothing in it streams. Every V4 Flash slot is pinned to `-0731`, the current V4 Flash build: the bare `deepseek/deepseek-v4-flash` slug still resolves to the older 0423 one (0.09/0.18, 1.0M context, no `parallel_tool_calls`), so it is strictly worse and unused. `low` puts `deepseek-v4.1-flash` (GA 2026-09-10) in the planner seat at 4x the fleet's price: τ² airline 76.7% (#16 on 2026-09-10 — above the `-0731` fleet's 73.2% and everything near its price), 1.0M context, a 0.02x cache read, first-party DeepSeek plus resellers (9 endpoints on 2026-09-11, up from 4 on launch day, when a 429 from all of them killed a run — `model_errors.py` now waits such throttles out and isolates a dead sub-agent). GA pricing is double the launch-day 0.15/0.60. Throughput is a routing matter, not a model one — first-party serves ~127 tok/s (p50), the cheapest reseller ~12 — which is what `DRA_PROVIDER_MIN_THROUGHPUT` is for (Provider routing, below). Until a streamed run proves it keeps its `tool_calls` it stays on the streaming denylist, so the planner narrates nothing live. `mid` puts the newest Gemini Flash (`gemini-3.8-flash`, same price as the 3.7 build that held τ² #2 overall) behind the planner, with `low`'s planner (`deepseek-v4.1-flash`) as its fleet and the shared utility floor. `high` swaps `claude-sonnet-5` for `gpt-5.6-sol` (same price and τ² score, +8.7pp GPQA, half the latency) over a `gemini-3.8-flash` fleet (its 3.7 predecessor out-scored the planner on τ² — intended, since the fleet makes the calls). The utility slot is `deepseek-v4-flash-0731` in every tier — the floor, picked for context length (1.31M) and price rather than reasoning, since its job is input-heavy map/extract. Compaction is `gpt-5.6-luna` in `low` and `mid` (an input-heavy call, so the cheapest credible input price) and the fleet model `gemini-3.8-flash` in `high`: the summary must keep every number and source for the planner and every sub-agent, and at a few compactions per run the price barely shows. The `high` coder is pinned to the GA `deepseek-v4-pro-0813` build, served first-party at 0.66/1.98 (the table shows the index price, a reseller's); the bare `deepseek-v4-pro` slug is the 0423 build on degraded third-party hosts. One model runs ahead of the benchmarks — `gemini-3.8-flash` (2026-09-02, same price as the 3.6/3.7 builds it replaced as `mid` planner and coder and `high` fleet and compactor); re-check it when the data lands.

**Provider routing (speed, price cap, stability).** OpenRouter's default routing is price-weighted, and a model's cheapest providers run 5–10x slower than its fastest (2026-09-11: the fleet model's $0.05–0.07 endpoints at 16–55 tok/s vs 115–117 at $0.10–0.13). Every ReAct step pays that, so `models.py` sends a `provider` object with every call, built per model at graph start (`provider_routing.py`) from OpenRouter's public endpoint feed (`GET /api/v1/models/{slug}/endpoints`, cached `DRA_PROVIDER_ROUTING_TTL` s). Three rules, in order of force: **`max_price`** = `DRA_PROVIDER_MAX_PRICE_FACTOR` (1.25) x the cheapest *healthy* endpoint's price, per axis in $/M — anything pricier is refused even when the cheap one is slow (hard: OpenRouter errors rather than falls back, which is why the baseline is re-read every TTL); **`ignore`** = providers with no endpoint at `DRA_PROVIDER_MIN_UPTIME` (97%) over the last 30 min or flagged down (OpenRouter itself only skips outages from the last 30 s); **`preferred_min_throughput`** (`DRA_PROVIDER_MIN_THROUGHPUT`, 50 tok/s at p50) and optional `preferred_max_latency` — soft, they push slow endpoints to the back among what remains. `DRA_PROVIDER_SORT=throughput|latency|price` is the hard sort override (balancing off, price ignored). Feed unreachable → soft preferences only, logged. Consequence worth knowing: when a model's cheapest healthy provider is also its slowest — `deepseek-v4.1-flash`'s $0.20 reseller at 12 tok/s next to $0.30 first-party at 127 — the cap pins traffic to it; a factor of 1.5 admits the $0.30 tier.
...
| `script` | `id`, `agent`, `name` (basename), `language`, `code`, `truncated` (+ `output`, `output_truncated` for inline code) | **Collapsed "view script" tab** holding code an agent ran — the ONLY place a script surfaces. Two sources, same event: a FILE script is emitted once at the sub-agent's handoff carrying the final version (initial `write_file` plus every later `edit_file` replayed); INLINE code (a `python3 - <<'PY'` heredoc, or `python3 -c "…"`) is emitted when its `execute` call returns, with `name` = `inline.py` and `output` = the real printed result. `agent` is the role that ran it — `orchestrator` included, since it computes with `execute` and holds no file tools |
...
- The same holds for the **model** side: a 429 / 5xx / timeout from the model provider is waited out within `model_rate_limit_max_wait` (`status: rate_limited` per wait), and a sub-agent that still dies mid-`task` comes back to the orchestrator as an error tool result (`status: subagent_failed`) — it retries the unit or reports the gap; one throttled sub-agent model no longer ends the run.
...
- **Coding sub-agent.** With a sandbox attached, every tool-loop role (orchestrator and research-subagents) can hand a programming job to `coding-subagent` via `task`: write a Python script for a goal over named `/workspace` files, or fix code given the code and the exact error. It runs on the tier's `coding_model` (a dedicated coder), keeps `write_file`/`edit_file`/`read_file`/`execute` but no data tools, and hands back `STATUS / OUTPUT / NOTES` — the script's *real* output, no narration. The prompts route every failed `execute` there and forbid `python -c "…"` one-liners (shell quoting is what breaks them — scripts are files), which is what ends the "I will now try a safer quoting approach…" loops on weak models.
- **Scripts are artifacts, not paths.** No role ever names a script in prose. The code goes to the frontend on its own `script` event (basename, language, final source) for a collapsed "view script" tab, emitted by `script_artifacts.py` from the sub-agent's own `write_file` / `edit_file` calls — app-code channel, never model context. Code run INLINE through `execute` (a heredoc, or the `python3 -c` a model reaches for anyway) writes no file, and `execute` is a deepagents built-in that `instrument_tool` never wraps — so `ExecuteArtifactsMiddleware` emits that code, plus its real output, on the same event for every role including the orchestrator. Without it a computed number reaches the user with no evidence anything ran. The coder's handoff is then scrubbed deterministically (`SCRIPT:` line, `/workspace/...` paths, bare `name.py` mentions) *before* it becomes the orchestrator's tool result, because a path the planner reads is a path the report repeats — and the reader can neither open the sandbox nor find it after the run ends. One exception survives the scrub: a labeled `RESULT FILE:` line, the output file too large to print that the caller must hand to the extract worker. `report_hygiene.scrub_report` remains the last-mile net on the report itself.
...
src/deep_research_agent/
  ...
  triage.py           pre-triage router — SIMPLE questions answered in two tiny calls, no harness
                      (a computation is never SIMPLE: it needs Python, so it goes to the full agent)
  ...
  model_errors.py     budgeted backoff on throttled model calls; a dead sub-agent → error tool result
  provider_routing.py per-model OpenRouter `provider` object from the live endpoint feed (price cap, ignore list)
  ...
  script_artifacts.py scripts -> `script` event (UI tab): files at handoff, inline `execute`
                      code + its output as it runs; scrubs paths from the coder's handoff
